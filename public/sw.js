...
/**
 * Версия берётся из адреса воркера (?v=<отпечаток сборки>), который
 * подставляет ServiceWorkerRegistrar. Раньше здесь стояла строка "v1",
 * и очистка при активации не срабатывала никогда: она удаляет кэши, не
 * заканчивающиеся на текущую версию, а версия не менялась. Из-за этого
 * оболочка прошлой выкатки оставалась в кэше навсегда.
 */
const VERSION = new URL(self.location.href).searchParams.get("v") || "v1";

/**
 * Версионируется только оболочка: в ней лежат HTML, JS и CSS, которые
 * обязаны смениться вместе с выкаткой.
 *
 * Данные, медиа и тайлы намеренно живут в кэшах с постоянными именами.
 * Иначе каждый деплой стирал бы скачанный офлайн-пакет города: турист
 * загрузил Самарканд перед поездкой, а к моменту вылета вышло обновление —
 * и в дороге у него пусто. Ответы API и так освежаются
 * stale-while-revalidate, а тайлы карты слишком дороги, чтобы качать их
 * заново из-за правки вёрстки.
 */
const SHELL = `shell-${VERSION}`;
// Имена оставлены прежними намеренно: у тех, кто уже скачал города, пакеты
// лежат именно в них, и переименование стёрло бы их при первом обновлении.
const API = "api-v1";
  ...
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            // Сносим только оболочки прошлых версий. Кэши данных, медиа и
            // тайлов не трогаем — там лежат скачанные офлайн-пакеты городов.
