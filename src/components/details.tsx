    ...
    <div className="flex flex-col h-full animate-slide-up" style={{ background: CREAM }}>
      <div className="relative flex-shrink-0" style={{ height: 260 }}>
        ...
        <button
          ...
          className="absolute top-12 right-4 w-9 h-9 rounded-xl flex items-center justify-center backdrop-blur-sm"
          style={{ ...glass }}
        >
          <svg
            width="16"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <div className="flex items-center gap-1.5 mb-1.5">
            <Badge text={place.type} onPhoto />
            {записи.length > 0 && <Badge text={"🎧 " + t("d_audioguide")} onPhoto />}
            {place.qr && <Badge text="QR" onPhoto />}
          </div>
      ...
      <div className="flex-1 overflow-y-auto hide-scroll px-4 pt-4">
        <div className={`grid ${запись?.seconds ? "grid-cols-4" : "grid-cols-3"} gap-2 mb-4`}>
          {[
            {
              e: "📍",
              v: км != null ? дист.формат(км) : дист.изДанных(place.distance),
            ...
            <div
              key={s.k}
              className="bg-white rounded-2xl p-2.5 text-center shadow-sm border"
              style={{ borderColor: BORDER }}
            >
              <p className="text-base">{s.e}</p>
              <p className="font-semibold text-[10px] mt-1 leading-tight" style={{ color: TEXT }}>
                {s.v}
              </p>
          ...
          <div className="rounded-2xl p-4 mb-3" style={{ background: ACCENT_FILL }}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-semibold text-sm truncate">{трК(запись.title)}</p>
                <p className="text-white/60 text-xs">{трК(запись.lang)}</p>
              </div>
              {длительность > 0 && (
                <span className="text-white/50 text-xs">
                  {времяЗвука(позиция) || "0:00"} / {времяЗвука(длительность)}
                </span>
              )}
            </div>
            {/* Полоска — настоящая позиция звука; нажатие перематывает. */}
            <div
              className="rounded-full h-1.5 mb-3 cursor-pointer"
              style={{ background: "rgba(255,255,255,0.2)" }}
              onClick={(e) => {
                if (!этаИграет) return;
                const r = e.currentTarget.getBoundingClientRect();
                плеер.перемотать((e.clientX - r.left) / r.width);
              }}
            >
              <div
                ...
                style={{
                  background: GOLD,
                  width: `${длительность ? Math.min(100, (позиция / длительность) * 100) : 0}%`,
            ...
            >
              {этаИграет && плеер.играет ? "⏸ " + t("d_pause") : "▶ " + t("d_listen")}
            </button>
            {плеер.ошибка === "failed" && этаИграет && (
              <p className="text-xs mb-2" style={{ color: "#ffd9d4" }}>
                {t("audio_failed")}
              </p>
            )}
            {записи.length > 1 && (
        ...
        <div className="flex gap-3 mb-3">
          ...
          <button
            onClick={() => {
              const стало = переключитьВМаршруте({
                id: place.id,
                name: place.nameRu ?? place.name,
                city: place.city,
                img: place.img,
              });
              onToast(
                стало
                  ? `✅ «${place.name}» — ${t("trip_added")}`
                  : `✕ «${place.name}» — ${t("trip_removed")}`,
              );
            }}
          ...
          >
            {вМаршруте ? `✓ ${t("trip_in")}` : `🗺️ ${t("d_add_route")}`}
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto hide-scroll px-4 pt-4">
        ...
        <div className="bg-white rounded-2xl p-4 mb-3 shadow-sm border" style={{ borderColor: BORDER }}>
          ...
          <div className="flex items-center justify-between mb-3">
            {[
              { l: t("d_nights"), v: nights, set: setNights, min: 1 },
              { l: t("d_guests"), v: guests, set: setGuests, min: 1 },
            ].map((c) => (
              <div key={c.l}>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => c.set((n: number) => Math.max(c.min, n - 1))}
    ...
    <div className="flex flex-col h-full animate-slide-up" style={{ background: CREAM }}>
      ...
      <div className="flex-1 overflow-y-auto hide-scroll px-4 pt-4">
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            { e: "🕐", v: r.open, k: "d_mode" as const },
            { e: "💰", v: дг.цена(r.price), k: "d_price" as const },
            { e: "🍽️", v: r.cuisine, k: "i_cuisine" as const },
          ].map((s) => (
            <div
              key={s.k}
  ...
  return (
    <div className="flex flex-col h-full animate-slide-up" style={{ background: CREAM }}>
      <div className="relative px-4 pt-12 pb-5" style={{ background: route.color }}>
        <div className="absolute inset-0 flex items-center justify-end pr-4 opacity-15">
        ...
        <div className="relative z-10">
          ...
          <h2
            className="text-white text-xl font-bold mt-1 leading-tight"
            style={{ fontFamily: "var(--font-heading)" }}
          >
            {route.icon} {трК(route.title)}
          </h2>
          <p className="text-white/70 text-xs mt-1">{трК(route.sub)}</p>
          <div className="flex gap-4 mt-3">
            {[
              ["⏱", route.duration],
              ["📍", `${route.stops.length} ${t("d_stops_short")}`],
            ].map(([e, v]) => (
      ...
      <div className="flex-1 overflow-y-auto hide-scroll p-4">
          ...
          <div key={i} className="flex gap-3 mb-2">
            <div className="flex flex-col items-center">
              ...
              {i < route.stops.length - 1 && (
                <div className="w-0.5 flex-1 mt-1" style={{ background: BORDER }} />
              )}
            </div>
            <div
              className="bg-white rounded-2xl p-3 flex-1 shadow-sm border mb-2"
              style={{ borderColor: BORDER }}
            >
              ...
              {s.note && (
                <p className="text-xs mt-1.5 font-medium" style={{ color: route.color }}>
                  💡 {трК(s.note)}
                </p>
              )}
            </div>
        ...
        <div className="flex gap-3 mb-3 mt-2">
          <button
            onClick={() => {
              if (перваяТочка) onПуть(перваяТочка.name, "");
            }}
          ...
          <button
            onClick={() => {
              const стало = переключитьИзбранное({
                id: route.id,
                kind: "route",
                name: трК(route.title),
                city: трК(route.badge),
                img: "",
                rating: 0,
              });
              onToast(стало ? `💾 «${трК(route.title)}» — ${t("d_save")}` : `✕ «${трК(route.title)}»`);
            }}
            className="flex-1 py-3.5 rounded-2xl text-sm font-bold border transition-all active:scale-[0.98]"
            style={{
              color: route.color,
              borderColor: route.color,
              background: сохранён ? мягко(route.color) : SURFACE,
            }}
          >
            {сохранён ? "✓" : "💾"} {t("d_save")}
          </button>
