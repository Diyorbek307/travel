...
export default function RealMap({
  ...
  откуда?: Geo | null;
  /**
   * Точка, к которой нужно подъехать вплотную. Меняется — карта плавно
   * центрируется на ней с крупным зумом. Так «где я» не просто ставит
   * синюю точку где-то на общей карте страны, а показывает её вблизи.
   */
  фокус?: Geo | null;
  /** Линия дороги, если её посчитал движок маршрутизации. */
  путь?: Geo[] | null;
  /**
   * Высота карты. Кроме числа принимает строку CSS — на ноутбуке карта
   * должна занимать больше места, чем на телефоне, и решает это одна
   * строка clamp вместо замеров окна в коде.
   */
  ...
  // Подъехать к точке фокуса (нажали «где я»). Отдельно от отрисовки:
  // зум — это разовое действие по команде, а не часть перерисовки меток.
  useEffect(() => {
    if (!фокус) return;
    карта.current?.setView([фокус.lat, фокус.lon], 15, { animate: true });
  }, [фокус]);
