/**
 * Узор поверх снимка в шапке — перенесён из исходников макета.
 *
 * Восьмиконечные звёзды исламской геометрии, ромбы и точки медленно
 * плывут, а две нити Шёлкового пути прочерчивают себя слева направо.
 * Всё в одном SVG с процентными координатами, поэтому слой ничего не
 * весит и тянется под любую высоту шапки.
 */

/** Восьмиконечная звезда — тот же мотив, что в плашке вместо фотографии. */
const STAR =
...
export default function HeroPattern() {
  return (
    <svg
      ...
      aria-hidden
    >
      {/* Две нити Шёлкового пути, прочерчивающие себя. */}
      <path
        d="M 0,60 Q 15,45 30,50 Q 50,55 65,42 Q 80,30 100,38"
        fill="none"
        stroke="rgba(233,196,106,0.25)"
        strokeWidth="0.4"
        className="silk-road-path"
      />
      <path
        d="M 0,72 Q 20,58 45,65 Q 70,72 100,55"
        fill="none"
        stroke="rgba(46,125,90,0.2)"
        strokeWidth="0.3"
        className="silk-road-path"
      ...
      {SHAPES.map((s, i) => (
        ...
        >
          <path
            d={STAR}
            fill={i % 2 === 0 ? "rgb(233 196 106)" : "rgb(255 255 255)"}
            opacity={s.opacity}
