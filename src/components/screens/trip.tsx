    ...
    <div className="flex h-full flex-col animate-slide-up" style={{ background: CREAM }}>
      <div className="flex-shrink-0 border-b bg-white px-4 pb-4 pt-14" style={{ borderColor: BORDER }}>
        <div className="flex items-center gap-3">
          ...
          {маршрут.length > 0 && (
            <button
              onClick={очиститьМаршрут}
              className="flex-shrink-0 text-xs font-semibold transition-all active:scale-95"
              style={{ color: MUTED }}
            >
              {t("trip_clear")}
            </button>
        ...
        {маршрут.map((точка, i) => {
          ...
          return (
            ...
            >
              ...
              <button
                onClick={() =>
                  переключитьВМаршруте({ id: точка.id, name: точка.name, city: точка.city, img: точка.img })
                }
