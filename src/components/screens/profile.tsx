...
import { ACHIEVEMENTS, STAMPS, type УсловиеДостижения } from "@/data/content";
...
function Toggle({ on, set }: { on: boolean; set: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ background: on ? ACCENT_FILL : BORDER }}
    >
      <div
...
function Row({
  ...
  return onClick ? (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 py-3 border-b last:border-0 active:opacity-60 transition-opacity"
...
export function SettingsView({
  ...
  const поделиться = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "HelloUZ", url });
        return;
      }
  ...
  const скачатьДанные = () => {
    const данные = {
      ...
      настройки: (() => {
        try {
          return JSON.parse(localStorage.getItem("uzup.settings") || "{}");
        } catch {
          return {};
        }
      })(),
      избранное: (() => {
        try {
          return JSON.parse(localStorage.getItem("uzup.favorites") || "[]");
        } catch {
          return [];
        }
      })(),
      маршрут: (() => {
        try {
          return JSON.parse(localStorage.getItem("uzup.trip") || "[]");
        } catch {
          return [];
        }
      })(),
      посещения: (() => {
        try {
          return JSON.parse(localStorage.getItem("uzup.visits") || "{}");
        } catch {
          return {};
        }
      })(),
    ...
    const url = URL.createObjectURL(
      new Blob([JSON.stringify(данные, null, 2)], { type: "application/json" }),
    );
    const a = document.createElement("a");
    a.href = url;
    a.download = "hellouz-my-data.json";
  ...
  const сохранитьПрофиль = async () => {
    try {
      const res = await fetch("/api/auth/me", {
        ...
        body: JSON.stringify(новоеФото ? { ...форма, photo: новоеФото } : форма),
      });
  ...
  return (
    <div className="flex-1 overflow-y-auto hide-scroll p-4 animate-fade-in space-y-3">
      {!isPremium && (
        ...
        >
          ...
          <div className="flex-1">
            <p className="text-white font-bold text-sm">HelloUZ Premium</p>
            <p className="text-white/60 text-xs">{t("pay_no_ads")} · HelloUZ Pro</p>
          </div>
          <div
            className="px-3 py-1.5 rounded-xl text-xs font-bold"
            style={{ background: GOLD, color: ON_GOLD }}
          >
            39 000 {t("cur_uzs_word")}
          </div>
        </button>
      )}
      {isPremium && (
        ...
        >
          ...
          <div>
            <p className="text-white font-bold text-sm">{t("prof_premium_active")}</p>
            {user?.premiumUntil && (
              <p className="text-white/50 text-xs">
                {t("prem_until")} {датаСловами(new Date(user.premiumUntil), lang, "long")}
              </p>
            )}
          </div>
          <svg
      ...
      <div className="bg-white rounded-2xl px-4 shadow-sm border" style={{ borderColor: BORDER }}>
        <p className="font-bold text-xs pt-3 pb-2 uppercase tracking-widest" style={{ color: MUTED }}>
          {t("prof_language")}
        </p>
          ...
          {LOCALES.map((код) => {
            ...
            return (
              <button
                ...
                style={выбран ? { background: ACCENT_SOFT, borderColor: GREEN } : { borderColor: BORDER }}
              >
                <span className="text-base leading-none">{флаг}</span>
                <span className="text-xs font-medium truncate" style={{ color: выбран ? GREEN : TEXT }}>
                  {имя.join(" ")}
                </span>
                {выбран && (
                  <svg
          ...
          right={
            <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: BORDER }}>
              {(["metric", "imperial"] as const).map((u) => (
                <button
                  key={u}
                  onClick={() => задатьНастройку("units", u)}
      ...
      <div className="bg-white rounded-2xl px-4 shadow-sm border" style={{ borderColor: BORDER }}>
        ...
        <Row
          ...
          right={
            <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: BORDER }}>
              {(
                [
                  ["system", t("s_theme_system")],
                  ["light", t("s_theme_light")],
                  ["dark", t("s_theme_dark")],
                ] as const
              ).map(([v, ярлык]) => (
                <button
                  key={v}
                  onClick={() => задатьНастройку("theme", v)}
        ...
        <Row
          ...
          right={
            <select
              value={нст.currency}
              onChange={(e) => задатьНастройку("currency", e.target.value)}
              className="text-xs font-bold px-2 py-1 rounded-lg outline-none border"
              style={{ color: GREEN, borderColor: BORDER, background: CREAM }}
            >
              {["USD", "EUR", "RUB", "GBP", "KRW", "CNY", "JPY", "UZS"].map((c) => (
      ...
      <div className="bg-white rounded-2xl px-4 shadow-sm border" style={{ borderColor: BORDER }}>
        ...
        <Row
          icon="🔄"
          label={t("s_check_updates")}
          onClick={обновление === "available" ? () => location.reload() : проверитьОбновления}
          right={
      ...
      <button
        onClick={onLogout}
        className="w-full py-3.5 rounded-2xl text-sm font-bold border mb-1 active:scale-[0.98] transition-all"
        style={{
          color: "#E7574C",
          borderColor: "color-mix(in srgb,#E7574C 35%,transparent)",
          background: "color-mix(in srgb,#E7574C 12%,transparent)",
        }}
      >
        🚪 {t("prof_logout")}
      </button>
      ...
      {сообщение && (
        <div
          className="fixed left-1/2 bottom-24 z-[70] -translate-x-1/2 rounded-full px-4 py-2 text-sm font-semibold shadow-lg"
          style={{ background: TEXT, color: CREAM }}
        >
          {сообщение}
        </div>
      )}
      ...
      {панель && (
        <div
          ...
          onClick={() => setПанель(null)}
        >
          <div
            className="w-full sm:max-w-md max-h-[85dvh] overflow-y-auto rounded-t-3xl sm:rounded-3xl p-5"
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => setПанель(null)}
                className="text-xl opacity-50 active:opacity-100"
                style={{ color: TEXT }}
              >
                ×
              </button>
            ...
            {панель === "edit" && (
              <div className="space-y-3">
                ...
                <div className="flex flex-col items-center gap-2 pb-1">
                  <button
                    onClick={() => файлФото.current?.click()}
                    className="h-20 w-20 overflow-hidden rounded-full flex items-center justify-center active:scale-95 transition-transform"
                    style={{ background: ACCENT_SOFT, border: `1px dashed ${GREEN}` }}
                  >
                    {новоеФото ? (
                ...
                {(
                  [
                    ["firstName", t("f_field_first")],
                    ["lastName", t("f_field_last")],
                    ["country", t("f_field_country")],
                    ["phone", t("f_field_phone")],
                  ] as const
                ).map(([k, label]) => (
                  <div key={k}>
                    <label
            ...
            {панель === "delete" && (
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={удалитьАккаунт}
                    className="flex-1 py-3 rounded-xl text-sm font-bold text-white active:scale-[0.98]"
...
export function ProfileScreen({
  ...
  user: PublicUser | null;
  isPremium: boolean;
  startView?: "passport" | "bookings" | "support" | "stats" | "settings";
}) {
  /*
  ...
  const номерПаспорта = user
    ? `UZT-${new Date(user.createdAt).getFullYear()}-${user.id
        .replace(/[^a-z0-9]/gi, "")
        .slice(-4)
        .toUpperCase()}`
    : "UZT-—";
  ...
  const [view, setView] = useState<"passport" | "bookings" | "support" | "stats" | "settings">(
    startView ?? "passport",
  );
    ...
    <div className="flex flex-col h-full w-full max-w-xl mx-auto" style={{ background: CREAM }}>
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          {!isPremium && (
            <button
              ...
              style={{ background: `linear-gradient(135deg,#0a1f20,#0e3b38)`, color: GOLD }}
            >
              👑 Pro
            </button>
          )}
        ...
        <div className="flex gap-1.5">
          {TABS.map(([v, e, l]) => (
            <button
              key={v}
              onClick={() => setView(v)}
              className="flex-1 py-2 rounded-xl text-[10px] font-semibold flex flex-col items-center gap-0.5"
              style={
                view === v ? { background: ACCENT_FILL, color: WHITE } : { background: CREAM, color: MUTED }
              }
            >
              <span>{e}</span>
              <span>{t(l)}</span>
      ...
      {view === "passport" && (
        <div className="flex-1 overflow-y-auto hide-scroll animate-fade-in">
          ...
          <div className="px-4">
            <div
              className="rounded-3xl overflow-hidden mb-4 shadow-lg"
              style={{ background: `linear-gradient(135deg,${ACCENT_DEEP} 0%,${ACCENT_FILL} 100%)` }}
            >
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-[9px] font-bold tracking-widest uppercase" style={{ color: GOLD }}>
                      HelloUZ · Uzbekistan Travel
                    </p>
                    <p
                      className="text-white text-xl mt-0.5"
                      style={{ fontFamily: "var(--font-heading)", fontWeight: 600 }}
                    >
                      {t("prof_digital_passport")}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-white/40 text-[9px]">{t("prof_passport_no")}</p>
                    <p className="text-[10px] font-mono font-bold" style={{ color: GOLD }}>
                      {номерПаспорта}
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-white/60 text-xs">
                      {заработано}/{всегоШтампов} · {t("prof_stamps")}
                    </p>
                  </div>
                  <div className="ml-auto flex-shrink-0">
                    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
                      <circle cx="20" cy="20" r="18" stroke="rgba(255,255,255,0.18)" strokeWidth="2" />
                      <circle
                        cx="20"
                        cy="20"
                        r="18"
                        ...
                        strokeDashoffset={2 * Math.PI * 18 * (1 - процентШтампов / 100)}
                        transform="rotate(-90 20 20)"
                      />
                      <text x="20" y="25" textAnchor="middle" fill={GOLD} fontSize="11" fontWeight="bold">
                        {процентШтампов}%
                      </text>
                    </svg>
                  </div>
              ...
              <div className="h-6 flex border-t" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
                {Array.from({ length: 20 }).map((_, i) => (
                  <div key={i} className="flex-1 flex items-center justify-center" style={{ opacity: 0.22 }}>
              ...
              {штампы.map((s, i) => (
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: s.earned ? WHITE : MUTED }}
                  >
                    {трК(s.name)}
                  </p>
                  <p className="text-[8px] mt-0.5" style={{ color: s.earned ? GOLD : "#C0B0A0" }}>
                    {s.earned ? s.date : трК("Не посещено")}
                  </p>
                </div>
              ))}
            </div>
            <div className="bg-white rounded-2xl p-4 mb-3 shadow-sm border" style={{ borderColor: BORDER }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-sm font-bold" style={{ color: GREEN }}>
                  {заработано}/{всегоШтампов}
                </p>
              </div>
              <div className="rounded-full h-2" style={{ background: CREAM }}>
                <div
                  className="h-2 rounded-full"
                  style={{ background: ACCENT_FILL, width: `${процентШтампов}%`, transition: "width 0.4s" }}
                />
              </div>
            ...
            <div className="grid grid-cols-3 gap-2.5 pb-4">
              {ACHIEVEMENTS.map((a) => {
                const есть = выполнено[a.условие];
                return (
                  <div
                    key={a.title}
                    className="bg-white rounded-2xl p-3 text-center shadow-sm border"
                    style={{ borderColor: BORDER, opacity: есть ? 1 : 0.55 }}
                  >
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-1.5 mx-auto"
                      style={{ background: мягко(a.color, 10) }}
                    >
                      {a.emoji}
                    </div>
                    <p className="text-[9px] font-semibold leading-tight" style={{ color: TEXT }}>
                      {трК(a.title)}
                    </p>
                    <p className="text-[8px] mt-0.5" style={{ color: есть ? GREEN : MUTED }}>
                      {есть ? `✓ ${t("ach_earned")}` : t("ach_progress")}
                    </p>
                  </div>
                );
              })}
        ...
        <div className="flex-1 overflow-y-auto hide-scroll animate-fade-in">
          ...
          <div className="px-4 space-y-4 pb-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                { e: "🏙️", v: String(Object.keys(визиты).length), l: t("prof_cnt_cities") },
                { e: "🏅", v: `${заработано}/${всегоШтампов}`, l: t("prof_cnt_stamps") },
                { e: "❤️", v: String(избранноеСписок.length), l: t("prof_cnt_fav") },
                { e: "📋", v: String(маршрутСписок.length), l: t("prof_cnt_trip") },
              ].map((s) => (
                <div
                  key={s.l}
                ...
                активность.map(({ место, iso }) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm truncate" style={{ color: TEXT }}>
                        <span style={{ color: MUTED }}>{t("prof_act_opened")}</span> {место!.name}
                      </p>
                      <p className="text-xs" style={{ color: MUTED }}>
                        {трК(место!.city)} · {датаСловами(new Date(iso), lang, "short")}
                      </p>
                    </div>
                  </div>
                ))
