  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto hide-scroll" style={{ background: CREAM }}>
      ...
      <div className="relative h-[400px] flex-shrink-0 lg:h-[480px]">
        ...
        <div
          className="absolute top-16 right-4 rounded-2xl p-3 z-10"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              >
                {самарканд ? `${самарканд.temp}°C` : "—"}
              </p>
              <p className="text-[9px] mt-0.5" style={{ color: MUTED }}>
                {самарканд ? t(самарканд.condKey) : ""}
              </p>
              <p className="text-[9px]" style={{ color: MUTED }}>
                {самарканд ? `💨 ${самарканд.windKmh} ${t("w_wind")}` : ""}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-4 z-10">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-white/65 text-xs">
              {t("home_welcome")} · {датаСловами(new Date(), lang)}
            </p>
          </div>
          <h1
          ...
          >
            ...
            <span className="flex-1 text-left text-sm" style={{ color: "rgba(255,255,255,0.72)" }}>
              {t("home_search_ph")}
            </span>
            <div
          ...
          {HOTELS.map((h) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to top,rgba(0,0,0,0.95) 0%,rgba(0,0,0,0.2) 50%,transparent 100%)",
                }}
              />
              <div className="absolute top-3 left-3">
                <span
                  className="text-[8px] font-bold px-2 py-0.5 rounded-full"
                  style={{ background: "rgba(233,196,106,0.92)", color: ON_GOLD }}
                >
                  {h.tag}
                </span>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                ...
                <p
                  ...
                  style={{ fontSize: 13, fontFamily: "var(--font-heading)" }}
                >
                  {h.name}
                </p>
                <div
                  className="flex gap-3 pb-2 mb-2 border-b"
                  style={{ borderColor: "rgba(255,255,255,0.1)" }}
                >
                  <div>
                    <p className="text-white font-bold text-[10px]">{h.rating}★</p>
                    <p className="text-[7px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                      {t("card_rating")}
                    </p>
                  </div>
                  <div>
                    <p className="text-white font-bold text-[10px]">{h.reviews}</p>
                    <p className="text-[7px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                      {t("d_reviews_word")}
                    </p>
                  </div>
                  <div>
                    <p className="text-white font-bold text-[10px]">{h.facilities.length}</p>
                    <p className="text-[7px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                      {t("home_services")}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-white font-bold text-sm">{дг.цена(h.price)}</p>
                  </div>
                  <div
          ...
          {RESTAURANTS.slice(0, 8).map((r) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to top,rgba(0,0,0,0.96) 0%,rgba(0,0,0,0.15) 55%,transparent 100%)",
                }}
              />
              <div className="absolute top-3 left-3">
                <span
                  className="text-[8px] font-bold px-2 py-0.5 rounded-full"
                  style={{ background: "rgba(193,96,58,0.92)", color: WHITE }}
                >
                  {r.cuisine}
                </span>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                ...
                <p
                  className="text-white font-bold leading-tight mb-1.5"
                  style={{ fontSize: 12, fontFamily: "var(--font-heading)" }}
                >
                  {r.name}
                </p>
                <div
                  className="flex gap-3 pb-1.5 mb-1.5 border-b"
                  style={{ borderColor: "rgba(255,255,255,0.1)" }}
                >
                  <div>
                    <p className="text-white font-bold text-[10px]">{r.rating}★</p>
                    <p className="text-[7px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                      {t("card_rating")}
                    </p>
                  </div>
                  <div>
                    <p className="text-white font-bold text-[10px]">{r.open.split("–")[0]}</p>
                    <p className="text-[7px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                      {t("home_open_word")}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-white font-bold text-xs">{дг.цена(r.price)}</p>
                  </div>
                  <div
...
function EventCard({
  ...
  ev: { name: string; date: string; city: string; emoji: string; color: string; desc: string };
  t: (k: TKey) => string;
  трК: (s: string) => string;
  onToast: (m: string) => void;
}) {
  ...
  return (
    <div
      ...
      style={{ width: 200, background: ev.color, position: "relative" }}
    >
      <div className="absolute inset-0 flex items-center justify-end pr-2 opacity-10">
        <GeomPattern opacity={1} />
      </div>
      <div className="relative z-10 p-4">
        <div
          className="w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-2.5"
          style={{ background: `rgba(${тень},0.18)` }}
        >
          {ev.emoji}
        </div>
        <p className="font-bold text-sm leading-tight" style={{ color: текст }}>
          {трК(ev.name)}
        </p>
        <p className="text-[9px] mt-1 font-semibold" style={{ color: текст, opacity: 0.8 }}>
          {трК(ev.date)} · {трК(ev.city)}
        </p>
        <p className="text-[9px] mt-2 leading-relaxed" style={{ color: текст, opacity: 0.72 }}>
          {трК(ev.desc)}
        </p>
        <button
          onClick={() =>
            onToast(
              напомнитьОСобытии({ name: трК(ev.name), date: ev.date, city: трК(ev.city), desc: трК(ev.desc) })
                ? t("remind_saved")
                : t("remind_no_date"),
            )
          }
          className="mt-3 px-3 py-1.5 rounded-xl text-[9px] font-bold transition-all active:scale-95"
          style={{ background: `rgba(${тень},0.18)`, color: текст }}
        >
          📅 {t("home_remind")}
        </button>
      </div>
