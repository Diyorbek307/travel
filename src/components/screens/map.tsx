          ...
          <div className="p-4 space-y-3">
            {!generated?(
              <>
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm border" style={{borderColor:BORDER}}><p className="font-bold text-sm mb-3" style={{color:TEXT}}>{t("map_interests")}</p><div className="flex flex-wrap gap-2">{[["история","🏛️"],["мечети","🕌"],["еда","🍽️"],["природа","🌿"],["базары","🛍️"],["музеи","🏺"]].map(([k,e])=><button key={k} onClick={()=>toggleInt(k)} className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium" style={ints.includes(k)?{background:ACCENT_FILL,color:WHITE}:{background:CREAM,color:TEXT}}>{e} {трК(k)}</button>)}</div></div>
                <button onClick={()=>{setGenerating(true);setTimeout(()=>{setGenerating(false);setGenerated(true);},2000);}} disabled={generating} className="w-full py-4 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-70" style={{background:ACCENT_FILL,color:WHITE}}>
                  {generating?<><span className="inline-block animate-spin">⚙️</span>{t("map_building")}</>:`✨ ${t("map_create")}`}
                </button>
              </>
            ):(
              <div className="animate-slide-up">
                <div className="rounded-2xl p-4 mb-4" style={{background:ACCENT_FILL}}><p className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-1">{t("map_ai_route")} · {трК(city)}</p><p className="text-white text-lg font-bold" style={{fontFamily:"var(--font-heading)"}}>{трК("История + Архитектура + Кухня")}</p><p className="text-white/60 text-xs mt-1">{трК("5 мест · 12 км · ~$26")}</p></div>
                {AI_STEPS.map((s,i)=>(
                  <div key={i} className="flex gap-3 mb-2">
                    <div className="flex flex-col items-center"><div className="w-7 h-7 rounded-xl flex items-center justify-center text-white text-xs font-bold" style={{background:ACCENT_FILL}}>{i+1}</div>{i<AI_STEPS.length-1&&<div className="w-0.5 flex-1 mt-1" style={{background:BORDER}}/>}</div>
