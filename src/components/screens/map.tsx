...
export function MapScreen({
  ...
  const план = generated
    ...
    : null;
  const длительностьСтр = (ч: number) =>
    ч >= 1
      ? `${ч.toLocaleString(lang, { maximumFractionDigits: 1 })} ${t("common_h")}`
      : `${Math.round(ч * 60)} ${t("common_min")}`;
  return (
    <div className="flex flex-col h-full" style={{ background: CREAM }}>
      <div className="px-4 pt-14 pb-3 bg-white border-b" style={{ borderColor: BORDER }}>
        ...
        <h1 className="text-xl font-bold mb-3" style={{ color: TEXT, fontFamily: "var(--font-heading)" }}>
          {t("map_title")}
        </h1>
        <div className="flex gap-2">
          {(["map", "routes", "ai"] as const).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className="flex-1 py-2 rounded-xl text-xs font-semibold"
              style={
                mode === m ? { background: ACCENT_FILL, color: WHITE } : { background: CREAM, color: MUTED }
              }
            >
              {m === "map"
                ? `🗺️ ${t("map_tab_map")}`
          ...
          <div className="p-4 space-y-4">
            ...
            >
              <RealMap
                ...
                onВыбор={(т) => {
                  if (т.подпись) setCity(т.подпись);
                }}
              />
            </div>
              ...
              <div className="bg-white rounded-2xl p-4 shadow-sm border" style={{ borderColor: BORDER }}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className="text-2xl">{погода.get(city)!.icon}</span>
                    <p className="font-bold text-lg" style={{ color: TEXT }}>
                      {погода.get(city)!.temp}°C
                    </p>
                    <p className="text-[9px]" style={{ color: MUTED }}>
                      {t(погода.get(city)!.condKey)}
                    </p>
                  </div>
                </div>
          ...
          <div className="p-4 space-y-3">
            {ROUTES.map((r) => (
              <button
                key={r.id}
                onClick={() => onRoute(r)}
                className="w-full bg-white rounded-2xl p-4 shadow-sm border text-left active:scale-[0.98]"
                style={{ borderColor: BORDER }}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0"
                    style={{ background: мягко(r.color) }}
                  >
                    {r.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge text={трК(r.badge)} color={r.color} />
                    </div>
              ...
              <>
                <div className="bg-white rounded-2xl p-4 shadow-sm border" style={{ borderColor: BORDER }}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["Самарканд", "Бухара", "Хива", "Ташкент"].map((c) => (
                      <button
                        key={c}
                        onClick={() => setCity(c)}
              ...
              <div className="animate-slide-up">
                <div className="rounded-2xl p-4 mb-4" style={{ background: ACCENT_FILL }}>
                  <p className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-1">
                    {t("map_ai_route")} · {трК(city)}
                  </p>
                  <p className="text-white text-lg font-bold" style={{ fontFamily: "var(--font-heading)" }}>
                    {сБольшой(ints.length ? ints.map((и) => трК(и)).join(" + ") : трК(hours))}
                  </p>
                  {план && план.остановки.length > 0 && (
                    <p className="text-white/60 text-xs mt-1">
                      {план.остановки.filter((о) => о.место).length} {t("d_stops_short")}
                      {план.км != null ? ` · ${план.км.toLocaleString()} ${t("unit_km")}` : ""}
                      {план.вход > 0 ? ` · ~$${план.вход}` : ""}
                    </p>
                  )}
                </div>
                {план && план.остановки.length === 0 && (
                  ...
                  <div key={i} className="flex gap-3 mb-2">
                    <div className="flex flex-col items-center">
                      <div
                        ...
                        style={{ background: о.обед ? "#C1603A" : ACCENT_FILL }}
                      >
                        {о.обед ? "🍽" : i + 1 - все.slice(0, i).filter((x) => x.обед).length}
                      </div>
                      {i < все.length - 1 && (
                    ...
                    <button
                      onClick={() => о.место && onPlace(о.место)}
                      disabled={!о.место}
                      className="bg-white rounded-2xl p-3 flex-1 shadow-sm border mb-2 text-left"
                      style={{ borderColor: BORDER }}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="font-bold text-sm" style={{ color: TEXT }}>
                            {о.место ? о.место.name : `${t("map_lunch")} · ${о.обед!.name}`}
                          </p>
                          <p className="text-[10px] mt-0.5" style={{ color: MUTED }}>
                            ⏱ {длительностьСтр(о.часов)}
                            {о.место ? ` · ${о.место.type} · 🎫 ${о.место.entry}` : ` · ${о.обед!.cuisine}`}
                          </p>
                        </div>
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-2xl text-sm font-semibold border mt-1 mb-4"
                  style={{ color: GREEN, borderColor: GREEN, background: SURFACE }}
                >
                  ← {t("map_new_route")}
                </button>
              </div>
