...
export function SplashScreen({ onStart, onLogin }:{ onStart:()=>void; onLogin:()=>void }) {
  ...
  return (
    <div className="flex flex-col h-full relative overflow-hidden">
      ...
      <div className="relative z-10 flex items-center gap-3 px-6 pt-16"><span data-brand-logo className="inline-flex shrink-0"><LogoMark size={44} intro tone="#ffffff"/></span><div><p className="text-white text-2xl font-bold leading-none" style={{fontFamily:"'Fraunces',serif",textShadow:"0 2px 12px rgba(0,0,0,0.55)"}}>UzRoam</p><p className="text-white text-xs mt-1 font-medium" style={{textShadow:"0 1px 8px rgba(0,0,0,0.6)"}}>{t("splash_tagline")}</p></div></div>
  ...
  return (
    <div className="flex flex-col h-full animate-slide-up" style={{background:CREAM}}>
      <div className="px-5 pt-14 pb-5 bg-white border-b" style={{borderColor:BORDER}}>
        ...
        <h2 className="text-2xl font-bold" style={{color:TEXT,fontFamily:"'Fraunces',serif"}}>{t("onb_interests_q")}</h2>
      ...
      <div className="px-4 pb-8 pt-3">
        <button onClick={onDone} disabled={sel.length===0} className="w-full py-4 rounded-2xl text-white font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50" style={{background:GREEN}}>{t("onb_start")} 🚀</button>
