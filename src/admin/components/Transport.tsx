    ...
    <div className="p-4 sm:p-7">
        ...
        <div>
            ...
            {[
              {
                ...
                val: String(activeTaxis),
                sub: "через Яндекс",
                color: "var(--color-amber)",
              },
              {
                label: "АВИАРЕЙСОВ",
                val: String(flights.length),
                sub: "сегодня",
                color: "var(--color-teal)",
              },
              {
                label: "МАРШРУТОВ ПОЕЗДОВ",
                val: String(trains.length),
                sub: "активных сегодня",
                color: "var(--color-text)",
              },
              {
                label: "МАРШРУТОВ АВТОБУСОВ",
                val: String(buses.length),
                sub: "активных сегодня",
                color: "var(--color-text)",
              },
            ].map((s) => (
          ...
          >
            <Card className="p-5">
              ...
              <div
                className={`rounded-lg p-4 mb-4 ${yandexConnected ? "" : ""}`}
                style={{
                  background: yandexConnected
                    ? "rgba(42,141,122,0.08)"
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <div
                    ...
                    style={{ background: yandexConnected ? "var(--color-teal)" : "var(--color-amber)" }}
                  />
                ...
                <div className="text-xs" style={{ color: "var(--color-muted)" }}>
                  {yandexConnected
                    ? `Аккаунт: ${yandexLogin} · ${activeTaxis} активных поездок`
                    : "Подключите аккаунт Яндекс для предложения такси пользователям"}
                </div>
              </div>
              {yandexConnected ? (
                <div className="flex flex-wrap gap-2">
                  <Btn variant="ghost" small onClick={() => setYandexConnected(false)}>
                    Отключить
                  </Btn>
            ...
            <Card className="p-5">
              ...
              <div className="flex flex-col gap-2">
                {flights.slice(0, 4).map((f) => (
                  <div
                    key={f.id}
                    className="flex flex-wrap items-center gap-3 text-xs py-2"
                    style={{ borderBottom: "1px solid var(--color-border)" }}
                  >
                    <span
                      style={{ color: "var(--color-muted)", fontFamily: "var(--font-mono)", width: "56px" }}
                    >
                      {f.id}
                    </span>
                    ...
                    <div className="ml-auto">
                      <Badge label={STATUS_LABEL[f.status] ?? f.status} color={STATUS_COLOR[f.status]} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-2">
                {trains.slice(0, 4).map((t) => (
                  ...
                  >
                    ...
                    <div className="ml-auto">
                      <Badge label={STATUS_LABEL[t.status] ?? t.status} color={STATUS_COLOR[t.status]} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-2">
                {buses.map((b) => (
                  ...
                  >
                    ...
                    <div className="ml-auto">
                      <Badge
                        label={b.seats === 0 ? "распродан" : STATUS_LABEL[b.status] ?? b.status}
                        color={b.seats === 0 ? "rose" : STATUS_COLOR[b.status]}
                      />
                    </div>
                  </div>
                ))}
            ...
            >
              <div className="flex flex-wrap items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <div
                    className="text-xs"
                    style={{ color: yandexConnected ? "var(--color-teal)" : "var(--color-amber)" }}
                  >
              ...
              {!yandexConnected && (
                <p className="text-xs leading-relaxed" style={{ color: "var(--color-muted)" }}>
                  Подключите аккаунт Яндекс, чтобы пользователи могли заказывать такси прямо в приложении.
                  Поездки бронируются и оплачиваются через инфраструктуру Яндекса.
                </p>
              )}
            </div>
            ...
            {!yandexConnected ? (
              <div className="flex flex-col gap-3">
                <div>
                  <label
                    ...
                    style={{ color: "var(--color-muted)", fontFamily: "var(--font-mono)" }}
                  >
                    ЛОГИН ИЛИ EMAIL ЯНДЕКС
                  </label>
                  <input
                    type="text"
                    placeholder="yourname@yandex.com"
                    value={yandexLogin}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Btn variant="danger" onClick={() => setYandexConnected(false)}>
                    Отключить
                  </Btn>
                </div>
              </div>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="text-xs text-right shrink-0">
                      ...
                      <Badge
                        label={r.status === "in_ride" ? "в поездке" : "подъезжает"}
                        color={r.status === "in_ride" ? "teal" : "amber"}
                      />
                    </div>
                  </div>
                ))}
                  ...
                  {flights.map((f, i) => (
                    ...
                    >
                      ...
                      <td
                        ...
                        style={{
                          fontFamily: "var(--font-mono)",
                          color: f.seats === 0 ? "var(--color-rose)" : "var(--color-text)",
                        }}
                      >
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[34rem] text-sm">
                <thead>
                  ...
                  >
                    {["ПОЕЗД", "МАРШРУТ", "ДАТА", "ОТПР", "ПРИБЫТИЕ", "КЛАСС", "ЦЕНА", "МЕСТА", "СТАТУС"].map(
                      (c) => (
                        <th
                          key={c}
                          className="text-left px-4 py-3 font-medium"
                ...
                <tbody>
                  {trains.map((t, i) => (
                    <tr
                      key={t.id}
                      style={{
                        borderBottom: i < TRAINS.length - 1 ? "1px solid var(--color-border)" : "none",
                      }}
                    >
                      <td
                        className="px-4 py-3"
                      ...
                      <td className="px-4 py-3">
                        <Badge label={t.class} color={t.class === "business" ? "amber" : "dim"} />
                      </td>
                      <td
                        className="px-4 py-3"
                        style={{ fontFamily: "var(--font-mono)", color: "var(--color-teal)" }}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              {(
                [
                  ["id", "Номер рейса", "text"],
                  ["from", "Откуда (код)", "text"],
                  ["to", "Куда (код)", "text"],
                  ["airline", "Авиакомпания", "text"],
                  ["dep", "Отправление", "text"],
                  ["arr", "Прибытие", "text"],
                  ["price", "Цена ($)", "number"],
                  ["seats", "Мест", "number"],
                ...
                <div key={k}>
                  ...
                  <input
                    type={type}
                    value={newFlight[k as keyof typeof newFlight]}
                    onChange={(e) => setNewFlight((p) => ({ ...p, [k]: e.target.value }))}
                    className="w-full rounded px-3 py-2 text-sm outline-none"
                    style={{
                      background: "var(--color-surface)",
                      border: "1px solid var(--color-border)",
            ...
            <div className="flex flex-wrap gap-3">
              <Btn variant="ghost" onClick={() => setShowAddFlight(false)}>
                Отмена
              </Btn>
              <Btn
                onClick={() => {
                  ...
                  setFlights((prev) => [
                    ...prev,
                    {
                      ...
                      seats: Number(newFlight.seats) || 0,
      ...
      {showAddRoute && (
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              {(
                ...
                <div key={k}>
                  ...
                  <input
                    type={type}
                    value={newRoute[k as keyof typeof newRoute]}
            ...
            <div className="flex flex-wrap gap-3">
              <Btn variant="ghost" onClick={() => setShowAddRoute(null)}>
                Отмена
              </Btn>
              <Btn
                onClick={() => {
      ...
      {showYandexMgmt && (
        <div
          className="fixed inset-0 flex items-center justify-center z-50"
          style={{ background: "rgba(0,0,0,0.7)" }}
          onClick={() => setShowYandexMgmt(false)}
        >
          <div
          ...
          >
            ...
            <div className="flex flex-col gap-3 mb-5">
              {[
                ...
                <div
                  key={s.label}
                  className="flex flex-wrap justify-between gap-2 items-center py-2 text-sm"
                  style={{ borderBottom: "1px solid var(--color-border)" }}
                >
            ...
            <div className="flex flex-wrap gap-3">
              <Btn variant="ghost" onClick={() => setShowYandexMgmt(false)}>
                Закрыть
              </Btn>
              <Btn
                variant="danger"
