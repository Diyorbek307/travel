...
const KEYS: ApiKey[] = [
  {
    id: 1,
    name: "Приложение iOS",
    key: "hellouz_live_ios_k3f9...a2c1",
    created: "15 янв 2026",
    lastUsed: "2 мин назад",
    permissions: ["bookings:read", "users:read", "tours:read"],
    requests: 142840,
    status: "active",
  },
  {
    ...
    key: "hellouz_live_and_m7b4...f5e8",
    created: "15 янв 2026",
    lastUsed: "4 мин назад",
    permissions: ["bookings:read", "users:read", "tours:read"],
    requests: 118620,
    status: "active",
  },
  {
    id: 3,
    name: "Кабинет партнёров",
    key: "hellouz_live_prt_n2q1...d9k3",
    created: "3 мар 2026",
    lastUsed: "час назад",
    permissions: ["tours:read", "bookings:write", "hotels:read"],
    requests: 28410,
    status: "active",
  },
  {
    id: 4,
    name: "Вебхук аналитики",
    key: "hellouz_live_wbh_x8t5...z3p7",
    created: "10 апр 2026",
    lastUsed: "30 мин назад",
  ...
  {
    id: 5,
    name: "Старая CMS (отключается)",
    key: "hellouz_live_cms_r1w6...q4n2",
    created: "1 июн 2025",
    lastUsed: "45 дней назад",
    permissions: ["*"],
    requests: 0,
    status: "revoked",
  },
...
const INTEGRATIONS: Integration[] = [
  {
    name: "Яндекс Такси",
    logo: "🚕",
    desc: "Такси для трансферов из аэропорта и по городу",
    status: "connected",
    category: "Транспорт",
  },
  {
    name: "Google Maps",
    logo: "🗺️",
    desc: "Карты, маршруты и данные мест для страниц направлений",
    status: "connected",
    category: "Карты",
  },
  {
    name: "Stripe",
    logo: "💳",
    desc: "Обработка платежей за туры и подписки",
    status: "connected",
    category: "Платежи",
  },
  {
    name: "Firebase",
    logo: "🔔",
    desc: "Push-уведомления для iOS и Android приложений",
    status: "connected",
  ...
  },
  {
    name: "Twilio",
    logo: "📱",
    desc: "SMS-подтверждения и двухфакторная аутентификация",
    status: "connected",
    category: "Сообщения",
  },
  {
    name: "Mailchimp",
    logo: "📧",
    desc: "Email-кампании и письма-подтверждения бронирований",
    status: "error",
    category: "Email",
  },
  {
    name: "Uzbekistan Airways API",
    logo: "✈️",
    desc: "Живые данные о рейсах, поиск и бронирование",
    status: "connected",
    category: "Транспорт",
  },
  {
    ...
    logo: "🚆",
    desc: "Расписание поездов и продажа билетов",
    status: "connected",
    category: "Транспорт",
  },
  {
    name: "OpenWeather",
    logo: "🌤️",
    desc: "Погодные прогнозы для страниц направлений",
    status: "disconnected",
    category: "Данные",
  },
  {
    name: "Sentry",
    logo: "🐛",
    desc: "Отслеживание ошибок и мониторинг производительности",
    status: "connected",
    category: "DevOps",
  },
  {
    name: "Slack",
    logo: "💬",
    desc: "Оповещения и критические уведомления для администраторов",
    status: "disconnected",
    category: "Сообщения",
  },
  {
    ...
    logo: "☁️",
    desc: "Хранение фотографий направлений и ресурсов туров",
    status: "connected",
    category: "Хранилище",
  },
];
...
export default function Integrations() {
  ...
  const filteredIntegrations =
    catFilter === "Все" ? INTEGRATIONS : INTEGRATIONS.filter((i) => i.category === catFilter);
    ...
    <div className="p-4 sm:p-7">
            ...
            >
              <div className="flex flex-wrap items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <span
                      className="font-mono text-xs min-w-0 flex-1 truncate"
                      style={{ color: reveal === k.id ? "var(--color-amber)" : "var(--color-muted)" }}
                    >
                      {reveal === k.id ? k.key.replace("...", "••••••••••••") : k.key}
                    </span>
                    <span className="text-xs shrink-0" style={{ color: "var(--color-faint)" }}>
                      {reveal === k.id ? "скрыть" : "показать"}
                    </span>
                  </div>
                ...
                {k.status === "active" && (
                  <div className="flex flex-wrap gap-2 shrink-0">
                    ...
                    <Btn variant="danger" small onClick={() => revoke(k.id)}>
                      Отозвать
                    </Btn>
                  </div>
                )}
          ...
          <div className="flex gap-1.5 mb-5 flex-wrap">
            {categories.map((c) => (
              <button
                ...
                style={{
                  background: catFilter === c ? "var(--color-amber)" : "var(--color-panel)",
      ...
      {tab === "webhooks" && (
        <div className="flex flex-col gap-3">
          {[
            {
              url: "https://analytics.hellouz.uz/hooks/booking",
              events: ["booking.created", "booking.cancelled"],
              lastDelivery: "2 min ago",
              success: true,
            },
            {
              url: "https://erp.hellouz.uz/incoming/payment",
              events: ["payment.succeeded", "payment.failed", "refund.issued"],
              lastDelivery: "1h ago",
              success: true,
            },
            {
              url: "https://slack-proxy.hellouz.uz/alerts",
              events: ["review.flagged", "guide.application", "booking.cancelled"],
              lastDelivery: "3h ago",
              success: false,
            },
          ].map((wh, i) => (
              ...
              <div className="flex flex-wrap items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div
                    ...
                    style={{ color: "var(--color-muted)", fontFamily: "var(--font-mono)" }}
                  >
                    Последняя доставка: {wh.lastDelivery}
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 shrink-0">
                  <Btn variant="ghost" small>
          ...
          >
            ...
            <div className="flex flex-col gap-4 mb-5">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Например: Mobile App v2"
                  value={newKey.name}
                ...
                <div className="flex flex-wrap gap-2">
                  {PERMS.map((p) => (
                    <button
                      ...
                      style={{
                        background: newKey.perms.includes(p) ? "var(--color-amber)" : "var(--color-surface)",
                        color: newKey.perms.includes(p) ? "var(--color-on-accent)" : "var(--color-muted)",
                        border: "1px solid var(--color-border)",
                        fontFamily: "var(--font-mono)",
                      }}
                    >
