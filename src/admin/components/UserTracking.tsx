...
// Контур Узбекистана — упрощённый, точками для SVG
const UZ_BORDER =
  "90,80 180,60 280,50 380,40 450,50 520,60 590,70 650,80 700,100 720,130 710,160 700,200 720,240 700,280 680,300 650,320 600,340 550,360 480,380 420,400 380,420 340,440 290,450 240,440 190,430 150,420 110,400 80,370 60,330 50,280 55,230 65,180 75,130 90,80";
    ...
    <div className="p-4 sm:p-7">
      ...
      >
        ...
        >
          ...
          <svg viewBox="0 0 800 500" className="w-full h-full" style={{ minHeight: "480px" }}>
            ...
            {displayUsers.map((user) => {
              ...
              return (
                <g key={user.id}>
                  ...
                  {(() => {
                    const pos =
                      CITIES[user.location.city] ?? latLngToXY(user.location.lat, user.location.lng);
                    return (
                      <g
                      ...
                      >
                        <circle
                          ...
                          opacity="0.3"
                        />
                        <circle cx={pos.x} cy={pos.y} r="6" fill={color} stroke="#fff" strokeWidth="1.5" />
                        <text
                          x={pos.x}
                          y={pos.y + 4}
                          textAnchor="middle"
                        ...
                        {selectedUser?.id === user.id && (
                          <g>
                            <rect x={pos.x - 30} y={pos.y - 28} width="60" height="16" rx="4" fill={color} />
                            <text
                              x={pos.x}
                              y={pos.y - 17}
                              textAnchor="middle"
                ...
                {selectedUser.locationHistory.map((h, i) => (
                  <div key={i} className="flex flex-wrap gap-3 items-start pb-3 relative">
                    <div className="flex flex-col items-center">
                      ...
                      {i < selectedUser.locationHistory.length - 1 && (
                        <div
                          className="w-px min-w-0 flex-1 mt-1"
