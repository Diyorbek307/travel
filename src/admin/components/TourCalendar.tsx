    ...
    <div className="p-4 sm:p-7">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
        {[
          ...
          {
            label: "ЗАПОЛНЕННОСТЬ",
            val: `${Math.round((totalBooked / totalSeats) * 100)}%`,
        ...
        >
          ...
          {rows.map((week, wi) => (
            <div
              ...
              style={{
                borderBottom: wi < rows.length - 1 ? "1px solid var(--color-border)" : "none",
                minHeight: "100px",
              }}
            >
              {week.map((day, di) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      minHeight: "100px",
                      background: isToday ? "rgba(212,135,42,0.05)" : "transparent",
                    }}
                  >
                    {day > 0 && (
                      <>
                        ...
                        <div className="flex flex-col gap-0.5">
                          {dayEvents.slice(0, 3).map((ev) => (
                            <button
                              ...
                              style={{
                                background: мягко(ev.color, 13),
                                color: ev.color,
                                border: `1px solid ${мягко(ev.color, 20)}`,
                                fontFamily: "var(--font-mono)",
                                fontSize: "10px",
                              }}
                            >
          ...
          {events
            ...
            .map((ev) => (
              ...
              >
                ...
                <Badge
                  label={
                    { confirmed: "подтверждён", pending: "ожидание", cancelled: "отменён" }[ev.status] ??
                    ev.status
                  }
          ...
          >
            <div className="flex flex-wrap items-start gap-3 mb-5">
              <div
                ...
                style={{ background: selected.color, minHeight: "24px" }}
              />
              <div>
                <h3
              ...
              <Badge
                label={
                  { confirmed: "подтверждён", pending: "ожидание", cancelled: "отменён" }[selected.status] ??
                  selected.status
                }
                color={
                  selected.status === "confirmed" ? "teal" : selected.status === "pending" ? "amber" : "rose"
                }
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
              {[
                ...
                { label: "ЗАБРОНИРОВАНО", val: `${selected.booked} из ${selected.seats} мест` },
                {
                  label: "ДОСТУПНО",
                  val:
                    selected.seats - selected.booked > 0
                      ? `${selected.seats - selected.booked} мест свободно`
                      : "Заполнен",
                },
              ].map((f) => (
                <div
            ...
            <div className="mb-5">
              <div className="flex flex-wrap justify-between gap-2 text-xs mb-1.5">
                ...
                <span style={{ color: "var(--color-text)", fontFamily: "var(--font-mono)" }}>
                  {Math.round((selected.booked / selected.seats) * 100)}%
                </span>
              </div>
              <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--color-dim)" }}>
                <div
                  ...
                  style={{
                    width: `${(selected.booked / selected.seats) * 100}%`,
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {selected.status === "pending" && (
                <Btn onClick={() => confirmEvent(selected.id)}>Подтвердить</Btn>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  max={30}
                  value={newTour.start}
                  onChange={(e) => setNewTour((p) => ({ ...p, start: e.target.value }))}
                  className="w-full rounded px-3 py-2 text-sm outline-none"
                  style={{
                    background: "var(--color-surface)",
                    border: "1px solid var(--color-border)",
              ...
              <div>
                ...
                <input
                  ...
                  max={30}
                  value={newTour.end}
                  onChange={(e) => setNewTour((p) => ({ ...p, end: e.target.value }))}
                  className="w-full rounded px-3 py-2 text-sm outline-none"
                  style={{
                    background: "var(--color-surface)",
                    border: "1px solid var(--color-border)",
                ...
                onClick={() => {
                  ...
                  setEvents((prev) => [
                    ...prev,
                    {
                      ...
                      start: Number(newTour.start) || 1,
                      end: Number(newTour.end) || 1,
                      seats: Number(newTour.seats) || 10,
                      booked: 0,
