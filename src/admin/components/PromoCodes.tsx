...
const СТАТУС: Record<Promo["status"], { label: string; color: "teal" | "amber" | "dim" }> = {
  active: { label: "активный", color: "teal" },
  paused: { label: "приостановлен", color: "amber" },
  expired: { label: "истёк", color: "dim" },
};

const КАТЕГОРИЯ: Record<Promo["category"], string> = {
  all: "всё",
  tours: "туры",
  hotels: "отели",
  transport: "транспорт",
...
/** Полоса расхода: исчерпан — красная, почти исчерпан — янтарная. */
function цветРасхода(pct: number, обычный: string): string {
...
const PROMOS: Promo[] = [
  {
    id: 1,
    code: "SILKROAD20",
    description: "−20% на туры «Шёлковый путь» в сентябре",
    type: "percent",
    value: 20,
    minOrder: 200,
    used: 124,
  ...
  },
  {
    id: 2,
    code: "WELCOME50",
    description: "Приветственная скидка $50 на первую бронь",
    type: "fixed",
    value: 50,
    minOrder: 150,
    used: 89,
  ...
  },
  {
    id: 4,
    code: "FREETAXI",
    description: "Такси из аэропорта бесплатно при брони от $300",
    type: "free_transport",
    value: 0,
    minOrder: 300,
    used: 56,
  ...
  },
  {
    id: 5,
    code: "UPGRADE2026",
    description: "Номер классом выше без доплаты",
    type: "upgrade",
    value: 0,
    minOrder: 500,
    used: 22,
  ...
  },
  {
    id: 6,
    code: "SUMMER30",
    description: "Летняя акция: −30% на все туры",
    type: "percent",
    value: 30,
    minOrder: 250,
    used: 300,
  ...
  },
  {
    id: 7,
    code: "TRAIN10",
    description: "−10% на билеты на поезд",
    type: "percent",
    value: 10,
    minOrder: 20,
    used: 148,
...
export default function PromoCodes() {
  ...
  const процентные = promos.filter((p) => p.type === "percent");
  const средняяСкидка = процентные.length
    ? Math.round(процентные.reduce((s, p) => s + p.value, 0) / процентные.length)
    : 0;
  ...
  const addPromo = () => {
    const p: Promo = {
      ...
      status: "active",
      startDate: new Date().toLocaleDateString("ru", { day: "numeric", month: "short" }),
      endDate: newCode.endDate || "31 дек 2026",
      revenue: 0,
    };
    ...
    <div className="p-4 sm:p-7">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
        {[
          ...
          { label: "СРЕДНЯЯ СКИДКА", val: `${средняяСкидка}%`, color: "var(--color-text)" },
        ].map((s) => (
          <div
            key={s.label}
              ...
              <div className="flex flex-wrap items-start gap-4">
                ...
                >
                  <div
                    ...
                    style={{ color: "var(--color-amber)", fontFamily: "var(--font-mono)", fontSize: "13px" }}
                  >
                    {p.code}
                  </div>
                  <div className="text-xs mt-0.5 font-semibold" style={{ color: "var(--color-text)" }}>
                    {valueLabel(p)}
                  </div>
                </div>
                ...
                <div className="min-w-[11rem] flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <Badge label={СТАТУС[p.status].label} color={СТАТУС[p.status].color} />
                    <Badge label={КАТЕГОРИЯ[p.category]} color="dim" />
                  </div>
                  <div
                    className="text-xs flex flex-wrap gap-4 mb-2"
                    style={{ color: "var(--color-muted)", fontFamily: "var(--font-mono)" }}
                  >
                    <span>Мин. заказ: ${p.minOrder}</span>
                    <span>
                      Действует: {p.startDate} → {p.endDate}
                    </span>
                    {p.revenue > 0 && (
                      <span style={{ color: "var(--color-teal)" }}>
                        Выручка: ${p.revenue.toLocaleString()}
                      </span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span style={{ color: цветРасхода(pct, "var(--color-muted)") }}>
                        {p.used} / {p.limit} ({pct}%)
                      </span>
                    </div>
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{
                          width: `${pct}%`,
                          background: цветРасхода(pct, "var(--color-teal)"),
                        }}
                      />
                    </div>
                  </div>
                ...
                {p.status !== "expired" && (
                  <div className="flex flex-wrap gap-2 shrink-0">
                    <Btn
                      variant={p.status === "active" ? "danger" : "ghost"}
                      small
                      onClick={() => toggle(p.id)}
                    >
          ...
          >
            ...
            <div className="flex flex-col gap-3 mb-5">
              {[
                ...
                { key: "endDate", label: "Срок действия", ph: "31 дек 2026" },
              ].map((f) => (
                <div key={f.key}>
                  <label
                  ...
                  <input
                    type="text"
                    placeholder={f.ph}
                    value={newCode[f.key as keyof typeof newCode]}
                ...
                <div className="flex gap-2 flex-wrap">
                  {(
                    [
                      ["percent", "Процент"],
                      ["fixed", "Фикс. сумма"],
                      ["free_transport", "Бесплатное такси"],
                      ["upgrade", "Апгрейд"],
                    ] as [string, string][]
                  ).map(([t, tLabel]) => (
                    <button
                      key={t}
                      onClick={() => setNewCode((p) => ({ ...p, type: t }))}
                      className="px-2.5 py-1 rounded text-xs cursor-pointer"
                      style={{
                        background: newCode.type === t ? "var(--color-amber)" : "var(--color-surface)",
                        color: newCode.type === t ? "var(--color-on-accent)" : "var(--color-muted)",
                        border: "1px solid var(--color-border)",
                      }}
                    >
                      {tLabel}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(
                    ...
                    <button
                      key={c}
                      onClick={() => setNewCode((p) => ({ ...p, category: c }))}
                      className="px-2.5 py-1 rounded text-xs cursor-pointer"
                      style={{
                        background: newCode.category === c ? "var(--color-amber)" : "var(--color-surface)",
                        color: newCode.category === c ? "var(--color-on-accent)" : "var(--color-muted)",
                        border: "1px solid var(--color-border)",
                      }}
                    >
                      {cLabel}
