...
const CAMPAIGNS: Campaign[] = [
  {
    id: 1,
    title: "Сентябрьская скидка — туры -20%!",
    body: "Забронируйте тур мечты в Узбекистан в сентябре и сэкономьте 20%.",
  ...
  {
    id: 2,
    title: "Ваш тур в Самарканд начинается завтра!",
    body: "Готовьтесь к незабываемому путешествию. Проверьте маршрут.",
    segment: "Самарканд",
    sent: 64,
    opened: 61,
    clicked: 58,
  ...
  {
    id: 3,
    title: "Новинка: экспедиция на Аральское море",
    body: "Исследуйте древнее морское дно — мест мало. Успейте до 7 сен.",
    segment: "Искатели приключений",
    sent: 1840,
    opened: 620,
    clicked: 188,
    status: "sent",
  ...
  {
    id: 4,
    title: "Мы скучаем — возвращайтесь!",
    body: "Вы не бронировали 60 дней. Дарим скидку 10% на следующий тур.",
    segment: "Неактивные 60д",
    sent: 1220,
    opened: 341,
    clicked: 89,
    status: "sent",
  ...
  {
    id: 5,
    title: "Осень в Бухаре — новые даты",
    body: "Октябрьские туры в Бухару открыты. Ранняя цена скоро заканчивается.",
    segment: "Вишлист Бухара",
    sent: 0,
    opened: 0,
    clicked: 0,
    status: "scheduled",
  ...
  {
    id: 6,
    title: "Флэш-распродажа — только 48 часов!",
    body: "Скидка 40% на все транспортные бронирования только в эти выходные.",
    segment: "Все пользователи",
    sent: 0,
    opened: 0,
    clicked: 0,
    ...
    <div className="p-4 sm:p-7">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
        {[
          ...
          {
            label: "ОТКРЫТО",
            val: `${Math.round((totalOpened / totalSent) * 100)}%`,
            color: "var(--color-amber)",
          },
          {
            label: "КЛИКОВ",
            val: `${Math.round((totalClicked / totalSent) * 100)}%`,
        ...
        <div>
          <div className="flex flex-wrap gap-1.5 mb-4">
            {["all", "sent", "scheduled", "draft"].map((f) => (
              ...
              >
                {{ all: "Все", sent: "Отправлено", scheduled: "Запланировано", draft: "Черновик" }[f]}
              </button>
            ))}
          </div>
                ...
                <div className="flex flex-wrap items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      ...
                      <span>
                        {c.status === "scheduled" ? "Запланировано: " : "Отправлено: "}
                        {c.date}
                      </span>
                    </div>
                  </div>
                  ...
                  {c.status === "sent" && (
                    <div
                      className="flex flex-wrap gap-4 shrink-0 text-right text-xs"
                              ...
                              prev.map((x) =>
                                x.id === c.id
                                  ? {
                                      ...
                                      sent: SEGMENTS.find((s) => s.name === x.segment)?.count ?? 0,
          ...
          <Card className="p-5">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {["🌟", "🕌", "🏜️", "💛", "🍂", "⚡", "🎉", "✈️", "🚆", "🗺️"].map((e) => (
                    <button
                      key={e}
                      onClick={() => setCompose((p) => ({ ...p, emoji: e }))}
                      className="w-9 h-9 rounded-lg text-xl flex items-center justify-center cursor-pointer"
                      style={{
                        background: compose.emoji === e ? "var(--color-amber)" : "var(--color-surface)",
                        border: "1px solid var(--color-border)",
                      }}
                    >
                      {e}
                ...
                [
                  {
                    key: "title",
                    label: "ЗАГОЛОВОК",
                    ph: "Ваш тур в Самарканд начинается завтра!",
                    multiline: false,
                  },
                  {
                    key: "body",
                    label: "ТЕКСТ",
                    ph: "Готовьтесь к незабываемому путешествию...",
                    multiline: true,
                  },
                ] as const
              ).map((f) => (
              ...
              <div>
                ...
                <select
                  value={compose.segment}
                  onChange={(e) => setCompose((p) => ({ ...p, segment: e.target.value }))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["now", "schedule"].map((s) => (
                    <button
                      key={s}
                      onClick={() => setCompose((p) => ({ ...p, schedule: s }))}
                      className="px-3 py-1.5 rounded text-xs cursor-pointer capitalize"
                      style={{
                        background: compose.schedule === s ? "var(--color-amber)" : "var(--color-surface)",
                        color: compose.schedule === s ? "var(--color-on-accent)" : "var(--color-muted)",
                        border: "1px solid var(--color-border)",
                      }}
                    >
                      {s === "now" ? "Отправить сейчас" : "Запланировать"}
                    </button>
                  ))}
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap items-start gap-2">
                    ...
                    <div>
                      <div className="text-xs font-semibold text-white mb-0.5">
                        {compose.title || "Заголовок уведомления"}
                      </div>
                      <div className="text-xs" style={{ color: "rgba(255,255,255,0.65)", lineHeight: 1.4 }}>
                        {compose.body || "Текст уведомления появится здесь..."}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {SEGMENTS.map((s) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="font-medium text-sm mb-0.5" style={{ color: "var(--color-text)" }}>
                  {s.name}
                </div>
