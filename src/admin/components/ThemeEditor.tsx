  ...
  },
  {
    ...
    props: [
      ...
      },
      {
        ...
        label: "Радиус кнопки",
        type: "select",
        options: ["0px", "4px", "6px", "8px", "12px", "9999px"],
      },
      {
        key: "spacingBase",
        label: "Отступ страницы",
        type: "select",
        options: ["16px", "20px", "24px", "28px", "32px", "40px"],
      },
      {
        key: "sidebarWidth",
        label: "Ширина сайдбара",
        type: "select",
        options: ["192px", "208px", "224px", "240px", "256px"],
      },
    ],
  },
  {
...
const PRESETS: Array<{ name: string; theme: Partial<ThemeVars> }> = [
  {
    ...
    theme: {
      ...
      colorPanel: "#181520",
      colorBorder: "#26223a",
      colorAmber: "var(--color-amber)",
      colorTeal: "#18b89a",
      colorRose: "var(--color-rose)",
              ...
              {GROUPS.map((g) => (
                <button
                  ...
                  style={{
                    ...
                    borderLeft:
                      activeGroup === g.title ? "2px solid var(--color-amber)" : "2px solid transparent",
                    fontFamily: "var(--font-body)",
                  }}
                >
                  {g.title}
              ...
              <div className="flex flex-col gap-2">
                {currentGroup.props.map((prop) => (
                  ...
                  >
                    ...
                    <span style={{ color: "var(--color-faint)", fontSize: "12px" }}>
                      {selected === prop.key ? "▾" : "▸"}
                    </span>
                  </div>
                ))}
                  ...
                  <Card className="p-5">
                    ...
                    {prop.type === "select" && (
                      <div className="flex flex-wrap gap-2">
                        {prop.options?.map((opt) => (
                          <button
                            key={opt}
                            onClick={() => setVar(selected, opt)}
                            className="px-3 py-2 rounded text-xs cursor-pointer transition-all"
                            style={{
                              background:
                                theme[selected] === opt ? "var(--color-amber)" : "var(--color-surface)",
                              color:
                                theme[selected] === opt ? "var(--color-on-accent)" : "var(--color-muted)",
                              border: "1px solid var(--color-border)",
                              fontFamily: prop.key.includes("font")
                                ? opt.split(",")[0].replace(/'/g, "").trim()
                                : "var(--font-mono)",
                            }}
                          >
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Btn
                        variant="ghost"
                        small
                        onClick={() => setVar(selected, (DEFAULTS as ThemeVars)[selected])}
                      >
                        Сбросить
                      </Btn>
                      <Btn variant="ghost" small onClick={() => setSelected(null)}>
                        Готово
                      </Btn>
                    </div>
                  </Card>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      The Registan was the ancient heart of the Silk Road city of Samarkand, now in
                      Uzbekistan.
                    </p>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      Основной текст — читабелен в любом размере
                    </div>
                    <div
                      className="text-xs"
                      style={{ fontFamily: "var(--font-mono)", color: "var(--color-muted)" }}
                    >
                      MONO LABEL — BK-2640 — $890.00
                    </div>
                  </div>
                </div>
                    ...
                    {[
                      ...
                      <div key={c.label} className="flex flex-col items-center gap-1">
                        <div
                          ...
                          onClick={() => {
                            const propKey = Object.entries(theme).find(
                              ([, v]) => v === c.val,
                            )?.[0] as keyof ThemeVars;
          ...
          {PRESETS.map((preset) => (
            ...
            >
              <div
                ...
                style={{ background: preset.theme.colorBg ?? "var(--color-on-accent)" }}
              >
                <div
                  className="absolute left-0 top-0 bottom-0 w-16"
                  style={{
                    background: preset.theme.colorSurface ?? "#151410",
                    borderRight: `1px solid ${preset.theme.colorBorder ?? "var(--color-border)"}`,
                  }}
                >
                    ...
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        ...
                        style={{
                          background:
                            i === 1
                              ? preset.theme.colorAmber ?? "var(--color-amber)"
                              : preset.theme.colorDim ?? "#4a4237",
                          width: i === 1 ? "80%" : `${50 + i * 8}%`,
                        }}
                      />
                    ))}
                ...
                <div className="absolute left-20 top-3 right-3">
                  <div
                    ...
                    style={{ background: preset.theme.colorText ?? "var(--color-text)", opacity: 0.9 }}
                  />
                  <div className="flex flex-wrap gap-2 mb-2">
                    {[preset.theme.colorAmber, preset.theme.colorTeal, preset.theme.colorRose].map((c, i) => (
                      <div
                        key={i}
                        className="h-10 min-w-0 flex-1 rounded"
                        style={{
                          background: preset.theme.colorPanel ?? "#1c1a15",
                          border: `1px solid ${preset.theme.colorBorder ?? "var(--color-border)"}`,
                        }}
                      >
                        <div
                          className="h-1 mt-3 mx-2 rounded"
                          style={{ background: c ?? "var(--color-amber)", opacity: 0.8 }}
                        />
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-4" style={{ background: "var(--color-panel)" }}>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {[
                    preset.theme.colorAmber,
                    preset.theme.colorTeal,
                    preset.theme.colorRose,
                    preset.theme.colorPanel,
                    preset.theme.colorBg,
                  ].map((c, i) => (
                    <div key={i} className="w-4 h-4 rounded-sm" style={{ background: c ?? "#000" }} />
                  ))}
                </div>
                <Btn small onClick={() => applyPreset(preset)}>
                  Применить
                </Btn>
              </div>
            </div>
        ...
        <Card className="p-5 max-w-2xl">
          ...
          {history.length === 0 ? (
            <div className="text-sm text-center py-8" style={{ color: "var(--color-muted)" }}>
              История пуста — начните редактировать
            </div>
          ) : (
                    ...
                    {isColor ? (
                      <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                        <div
                          ...
                          style={{ background: h.from, border: "1px solid var(--color-border)" }}
                        />
                        <span style={{ color: "var(--color-faint)" }}>→</span>
                        <div
                          className="w-4 h-4 rounded-sm border"
                          style={{ background: h.to, border: "1px solid var(--color-border)" }}
                        />
                      </div>
                    ) : (
                      <div
                        className="text-xs flex flex-wrap gap-1.5 items-center shrink-0"
                        style={{ fontFamily: "var(--font-mono)", color: "var(--color-muted)" }}
                      >
                        <span>{h.from}</span>
                        <span style={{ color: "var(--color-faint)" }}>→</span>
