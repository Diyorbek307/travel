...
export default function Events() {
  ...
  const statusColor = (s: string) =>
    s === "upcoming" ? "teal" : s === "ongoing" ? "amber" : s === "past" ? "dim" : "rose";

  const catColors: Record<string, string> = {
    Festival: "var(--color-amber)",
    Concert: "var(--color-teal)",
    Exhibition: "var(--color-muted)",
    Sport: "var(--color-rose)",
    Cultural: "#7a5fd4",
    ...
    setEvents((prev) => [
      {
        ...
        emoji: "🎫",
        color: "#0E6F66",
        name: newEvent.name || "Новое событие",
        city: newEvent.city || "Ташкент",
        category: newEvent.category as Event["category"],
        date: newEvent.date,
        endDate: newEvent.date,
        venue: newEvent.venue || newEvent.city,
        capacity: Number(newEvent.capacity) || 500,
        ticketsSold: 0,
        price: Number(newEvent.price) || 0,
        status: "upcoming",
        featured: false,
        desc: "",
        img: "https://images.unsplash.com/photo-1664602078796-68ee76b3fc59?w=400&h=220&fit=crop&auto=format",
      },
      ...prev,
    ]);
    setShowForm(false);
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        ...
        {cities.map((c) => (
          <button
            ...
            style={{
              background:
                cityFilter === c ? "color-mix(in srgb, var(--color-amber) 15%, transparent)" : "transparent",
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${e.featured ? "rgba(212,135,42,0.5)" : "var(--color-border)"}`,
              }}
            >
              <div className="relative h-40 overflow-hidden" style={{ background: "var(--color-dim)" }}>
                ...
                <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                  <span
                    ...
                    style={{
                      background: мягко(catColors[e.category], 13),
                      color: catColors[e.category],
                      border: `1px solid ${мягко(catColors[e.category], 27)}`,
                      fontFamily: "var(--font-mono)",
                    }}
                  >
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Btn variant="ghost" small onClick={() => setDraft(e)}>
                    Изменить
                  </Btn>
        ...
        >
          ...
          >
                ...
                <div key={k}>
                  ...
                  <input
                    ...
                    onChange={(ev) =>
                      setDraft(
                        (d) =>
                          d && { ...d, [k]: type === "number" ? Number(ev.target.value) : ev.target.value },
                      )
                    }
            ...
            <div className="flex flex-wrap gap-2">
              <Btn
                onClick={() => {
                  setEvents((prev) => prev.map((ev) => (ev.id === draft.id ? draft : ev)));
                  setDraft(null);
                }}
              >
                Сохранить
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      style={{
                        background: newEvent.category === c ? "var(--color-amber)" : "var(--color-surface)",
                        color: newEvent.category === c ? "var(--color-on-accent)" : "var(--color-muted)",
                        border: "1px solid var(--color-border)",
                      }}
                    >
                      {label}
