...
function getTransport(): nodemailer.Transporter | null {
  ...
  transport = nodemailer.createTransport({
    host: HOST,
    port: PORT,
    // 465 — неявный TLS: шифрование с первого байта. Остальные порты
    // открывают соединение в открытую и поднимают шифрование через
    // STARTTLS уже внутри.
    secure: PORT === 465,

    /*
     * На нешифрованных портах требуем STARTTLS обязательно.
     *
     * Без этого nodemailer, не получив предложения о шифровании,
     * спокойно отправит письмо открытым текстом — вместе с логином и
     * паролем от ящика. Лучше внятный отказ, чем такая отправка.
     *
     * Заодно это ловит случай, когда на порту отвечает не почтовый
     * сервер: вместо молчаливого ожидания получаем ошибку.
     */
    requireTLS: PORT !== 465,
...
export async function проверитьПочту(): Promise<{ ok: boolean; detail: string }> {
   ...
   */
  /*
   * Сервер и ящик должны быть от одного почтовика.
   *
   * Реальная ошибка: SMTP_HOST указывал на smtp.yandex.ru, а ящик был
   * на mail.ru. Яндекс чужой адрес не пускает, письма молча не уходили,
   * и найти это глазами в списке переменных непросто.
   *
   * Ругаемся только когда домен принадлежит другому известному
   * почтовику. Свой домен на Яндекс 360 или Google Workspace — обычное
  ...
  const доменЯщика = (USER ?? "").split("@")[1]?.toLowerCase() ?? "";
  const серверПочтовик = ПОЧТОВИКИ.find((п) => (HOST ?? "").toLowerCase().includes(п.сервер));
  const ящикПочтовик = ПОЧТОВИКИ.find((п) => п.домены.includes(доменЯщика));

  if (серверПочтовик && ящикПочтовик && серверПочтовик !== ящикПочтовик) {
    return {
      ok: false,
      detail:
