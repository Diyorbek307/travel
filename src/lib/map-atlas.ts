...
/**
 * Настоящий рельеф страны.
 *
 * Карты высот и раскраски собирает scripts/geo/build-relief.py из данных
 * SRTM (public domain). Спутниковых снимков нет вовсе: цвет считается из
 * самих высот гипсометрией, поэтому лицензионных вопросов не возникает.
 */
export const RELIEF_BOX = BOUNDS;

export const RELIEF = {
  height: "/relief/uz-height.png",
  color: "/relief/uz-color.png",
  minMeters: -33,
  maxMeters: 4315,
} as const;

/**
 * Во сколько раз преувеличена высота.
 *
 * В истинном масштабе рельеф Узбекистана невидим: четыре с половиной
 * километра на полторы тысячи километров ширины дают в сцене четыре сотых
 * единицы — тоньше волоса. Рельефные карты преувеличивают всегда.
 *
 * Двадцать пять раз оказалось много: Гиссарский хребет вставал колючей
 * стеной и спорил с самой картой. Пятнадцать оставляют хребты читаемыми,
 * а страну — узнаваемой.
 */
export const RELIEF_EXAGGERATION = 15;

/** Размер растра рельефа в единицах сцены — по рамке страны. */
export const RELIEF_SIZE = {
  width: (BOUNDS.maxLon - BOUNDS.minLon) * LON_SQUEEZE * SCALE,
  depth: (BOUNDS.maxLat - BOUNDS.minLat) * SCALE,
};

/**
 * Полный размах высот в единицах сцены.
 *
 * Градус широты — это 111,32 км и ровно SCALE единиц, отсюда и пересчёт.
 * Сжатие по долготе здесь ни при чём: оно правит горизонталь, а не высоту.
 */
export const RELIEF_LIFT =
  ((RELIEF.maxMeters - RELIEF.minMeters) / 1000) * (SCALE / 111.32) *
  RELIEF_EXAGGERATION;
